/* @ts-self-types="./playground_wasm.d.ts" */
 ...
 * @returns {string}
 */
export function dump_celc(cache_bytes) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ptr0 = passArray8ToWasm0(cache_bytes, wasm.__wbindgen_malloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.dump_celc(ptr0, len0);
        deferred2_0 = ret[0];
        deferred2_1 = ret[1];
        return getStringFromWasm0(ret[0], ret[1]);
    } finally {
 ...
 * @returns {number}
 */
export function init_stdlib() {
    const ret = wasm.init_stdlib();
    return ret >>> 0;
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('playground_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
